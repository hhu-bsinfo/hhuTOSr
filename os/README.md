...
hhuTOS contains configuration files for VSCode and RustRover.

To use VSCode for Debugging, just install the _Rust_ and _C++_ extensions (_Memory Viewer_ is also recommended) and the debug target `qemu-gdb` should appear in the `Run and Debug` tab in VSCode. Just click the play button to start a debugging session.

To debug in _RustRover_, run the target _qemu-gdb_ and wait for the QEMU window to open. Afterward, run the _Start Debugger_ target by clicking the bug icon.
